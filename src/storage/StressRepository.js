...
/**
 * Validates stress settings
 * @param {object} settings - Settings to validate
 * @returns {object} Validation result
 */
export function validateStressSettings(settings) {
  const errors = [];

  if (settings.duration <= 0 || settings.duration > 100) {
    errors.push('Duration must be between 1 and 100 years');
  }
  if (settings.baseSalary < 0) {
    errors.push('Base salary cannot be negative');
  }
  if (settings.equityMin < 0 || settings.bondMin < 0 || settings.cashTarget < 0) {
    errors.push('Fund minimums cannot be negative');
  }
  if (settings.protectionMult < 0 || settings.protectionMult > 1) {
    errors.push('Protection multiplier must be between 0 and 1');
  }
  if (settings.statePensionYear < 0 || settings.statePensionYear > settings.duration) {
    errors.push('State pension year must be within duration');
  }
