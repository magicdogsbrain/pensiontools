...
/**
 * Calculates years until glidepath reaches zero (for growth funds)
 * @param {number} current - Current fund value
 * @param {number} minimum - Current glidepath minimum
 * @param {number} yearNumber - Current year number
 * @param {number} duration - Total duration
 * @returns {number} Years until minimum reaches zero
 */
export function yearsUntilGlidepathZero(current, minimum, yearNumber, duration) {
  return Math.max(0, duration - yearNumber);
}
