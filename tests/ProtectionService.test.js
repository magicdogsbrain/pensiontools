...
import {
  ...
  calculateProtectedDraw,
  determineWithdrawalSource,
  analyzeProtectionHistory,
  calculateProtectionShortfall,
...
describe('ProtectionService', () => {
  ...
  describe('calculateProtectedDraw', () => {
    it('should reduce draw by protection factor', () => {
      const result = calculateProtectedDraw(1000, 20);

      expect(result).toBe(800); // 20% reduction
    });

    it('should handle different protection factors', () => {
      expect(calculateProtectedDraw(1000, 10)).toBe(900);
      expect(calculateProtectedDraw(1000, 30)).toBe(700);
      expect(calculateProtectedDraw(1000, 50)).toBe(500);
    });

    it('should use default protection factor if not specified', () => {
      // Default is 20%
      const result = calculateProtectedDraw(1000);
  ...
  describe('analyzeProtectionHistory', () => {
    it('should return zeros for empty history', () => {
      const result = analyzeProtectionHistory([]);

      expect(result.totalProtectionMonths).toBe(0);
      expect(result.longestStreak).toBe(0);
      expect(result.protectionPeriods).toBe(0);
    });

    it('should count total protection months', () => {
      const history = [
        { inProtection: true },
        { inProtection: true },
        { inProtection: false },
      ...
      const result = analyzeProtectionHistory(history);

      expect(result.totalProtectionMonths).toBe(3);
    });

    it('should track longest protection streak', () => {
      const history = [
        { inProtection: true },
        { inProtection: true },
        { inProtection: true },
    ...
    it('should count separate protection periods', () => {
      const history = [
        { inProtection: true },
        { inProtection: true },
        { inProtection: false },
      ...
      expect(result.protectionPeriods).toBe(3);
    });
    ...
    it('should calculate percentage correctly', () => {
      ...
      expect(result.percentInProtection).toBe(50);
    });
  });

  describe('calculateProtectionShortfall', () => {
    it('should calculate total shortfall from protection periods', () => {
      const history = [
        { sipp: 2000, stdSipp: 2500, inProtection: true, boostAmount: 0 },
        { sipp: 2000, stdSipp: 2500, inProtection: true, boostAmount: 0 },
        { sipp: 2500, stdSipp: 2500, inProtection: false, boostAmount: 0 }
      ];

      const result = calculateProtectionShortfall(history);

      // Shortfall = (2500-2000) * 2 = 1000
      expect(result.totalShortfall).toBe(1000);
      expect(result.protectionMonths).toBe(2);
    });

    it('should subtract boost already taken', () => {
      const history = [
        { sipp: 2000, stdSipp: 2500, inProtection: true, boostAmount: 0 },
        { sipp: 2000, stdSipp: 2500, inProtection: true, boostAmount: 0 },
        { sipp: 2700, stdSipp: 2500, inProtection: false, boostAmount: 200 }
      ];

      const result = calculateProtectionShortfall(history);

      // Total shortfall = 1000, boost taken = 200
      expect(result.remainingShortfall).toBe(800);
    });
