...
vi.mock('../src/utils/DateUtils.js', () => ({
  ...
  getElapsedTaxYearMonths: vi.fn(() => 0)
}));
...
describe('DrawdownService', () => {
  ...
  describe('checkTaxEfficientMode', () => {
    beforeEach(() => {
      vi.mocked(getRemainingTaxYearMonths).mockReturnValue(12);
    });

    it('should confirm tax-efficient mode with sufficient ISA', () => {
      const result = checkTaxEfficientMode({
        currentDate: new Date('2024-04-01'),
        isaBalance: 12000,
        isaMonthlyDraw: 500,
        monthlyNetTarget: 3000,
        monthlySippNet: 2500,
        monthlyOtherNet: 0
      });

      // Need 500/month for 12 months = 6000
      expect(result.hasSufficientIsa).toBe(true);
      expect(result.taxEfficientMode).toBe(true);
      expect(result.isaShortfall).toBe(0);
    });

    it('should identify insufficient ISA for remaining months', () => {
      vi.mocked(getRemainingTaxYearMonths).mockReturnValue(10);
      ...
      // Need 500/month for 10 months = 5000, have 2000
      expect(result.hasSufficientIsa).toBe(false);
      expect(result.taxEfficientMode).toBe(false);
      expect(result.isaShortfall).toBe(3000);
    });

    it('should calculate correct monthly ISA need', () => {
      const result = checkTaxEfficientMode({
        currentDate: new Date('2024-04-01'),
        isaBalance: 0,
        isaMonthlyDraw: 0,
        monthlyNetTarget: 3500,
        monthlySippNet: 2500,
        monthlyOtherNet: 500
      });

      // Gap = 3500 - 2500 - 500 = 500/month
      expect(result.monthlyIsaNeed).toBe(500);
    });

    it('should return zero need when SIPP+Other covers target', () => {
      const result = checkTaxEfficientMode({
        currentDate: new Date('2024-04-01'),
        isaBalance: 1000,
        isaMonthlyDraw: 0,
      ...
      // Gap = 3000 - 2500 - 600 = -100, so 0
      expect(result.monthlyIsaNeed).toBe(0);
      expect(result.hasSufficientIsa).toBe(true);
    });
  });
  ...
  describe('calculateDrawdownRecommendation', () => {
    ...
    it('should use ISA as top-up when SIPP+Other would exceed BRL', () => {
      vi.mocked(getRemainingTaxYearMonths).mockReturnValue(12);

      const params = {
        ...baseParams,
        baseSalary: 55000,  // Target higher than BRL
        isaBalance: 50000   // Plenty of ISA
      };

      const result = calculateDrawdownRecommendation(params);

      expect(result.mode).toBe('tax-efficient');
      expect(result.isaUsedForTopUp).toBe(true);
      expect(result.monthlyIsa).toBeGreaterThan(0);
      expect(result.reason).toContain('ISA top-up');
    });

    it('should switch to standard mode when ISA insufficient', () => {
      vi.mocked(getRemainingTaxYearMonths).mockReturnValue(12);

      const params = {
        ...baseParams,
        baseSalary: 55000,  // Target higher than BRL
        isaBalance: 100     // Very little ISA
      };
      ...
      expect(result.monthlyIsa).toBe(0);
      expect(result.isaUsedForTopUp).toBe(false);
      expect(result.warning).toContain('more ISA');
    });
